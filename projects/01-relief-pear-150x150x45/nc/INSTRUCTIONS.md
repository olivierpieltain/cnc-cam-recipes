...
| 7 | `06_T1_CONTOUR_TRIM_PERIMETER.cnc`              | **T1** (back) Ø3.175 spiral O, 42 mm flute | −35 → **−45.3 mm** | final perimeter clearing — **adaptive** strategy (cloned from op 01A, follows the actual relief silhouette curves, not a square). Clears the 2 mm border between silhouette and stock perimeter, through the bottom 10 mm of the 45 mm physical stock + 0.3 mm into spoilboard. **Requires sacrificial spoilboard + heavy double-sided tape** (no fixturing). |
...
> **About T3:** Ø2.0 × **17 mm** flute is the physical bit you own. The
> Makera Carvera library only ships a 12 mm flute Ø2 ball, but the boxed
> Makera kit's only Ø2 *ball* is the 17 mm version (the 12 mm Ø2 in
> your kit is a **flat** endmill, in the 2-flute flat-endmill set
> `双刃平头铣刀套装`). The Fusion file uses a **custom library tool**
> ("2 Flute Ball Nose 2*17mm (Olivier physical)" in Local > Custom)
> generated by [`cam/create_t3_17mm_tool.py`](../cam/create_t3_17mm_tool.py)
> — this is more reliable than modifying the Makera library tool inline,
> which Fusion silently reverts on every doc load. The recipe
> ([`pear-2mm-ball-finishing-detail-rest.json`](../../../recipes/pear-2mm-ball-finishing-detail-rest.json))
> was originally tuned for the stiffer 12 mm flute, so expect slightly
> more deflection than the recipe nominally allows — scale the Carvera
> feed override down if it chatters. Broken Ø2 mm bits are easy, so be
> conservative on entry the first time.
...
| 8 (perimeter clearing) | part freed from surrounding stock — final piece with the relief on top + 10 mm of flat backing below; the 2 mm border around it is fully cleared (adaptive sweep, no leftover strips); whatever residue stays stuck to the spoilboard tape, peel off after |
...
- **For file 8 (perimeter clearing) WITHOUT fixtures:** adaptive
  clears the 2 mm border in a sweep so there are no full-length strips
  to fly off (a contour-2D approach left strips; this adaptive doesn't).
  Small chips lifting toward the cutter is still possible — if anything
  lifts, hit feed-hold immediately. With heavy tape coverage and
  air-assist clearing chips, this should not happen.
